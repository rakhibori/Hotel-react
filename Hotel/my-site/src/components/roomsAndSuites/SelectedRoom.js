...
const SelectedRoom = ({setFlagRezerv, selectedMyRoom, stayingTime}) => {
    ...
    const handlePlusCount = async(room)=>{ 
        var countRoom = 1;
        var countCapacity = 0;
        const temp = sampleRoom.filter(item => item.id === room.id)
                   ...
                   <>
                        <div className='selectedRoomBox'>
                            <div className='namePriceBox'>
                                ...
                                <div className='price'>{sampleRoom.map(item => item.id === room.id && item.qty * room.price * Number(stayingTime))} تومان</div>
                            </div>
                            <div className='changeCountBox'>
                                <button onClick={()=>handlePlusCount(room)}>+</button>
                                {
                                    sampleRoom.map(item => item.id === room.id && item.qty)     
                                } اتاق
