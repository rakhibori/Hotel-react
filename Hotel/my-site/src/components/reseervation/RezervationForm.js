...
const ReservationForm = ({selectedMyRoom, entryDate, exitDate, stayingTime, setBookersRoom}) => {
    const {data: rooms, isLoading} = useGetRoomsQuery();
    const [createUsersRooms] = useCreateUsersRoomsMutation();
    const form = useFormik({
        initialValues:{name:'', family:'', mobile:'', nationalCode:'', email:'', guestName: '', guestFamily: '' },
    ...
    const handleRezerv = () => {
        if(form.values.name !== '' & form.values.family !== '' & form.values.mobile !== ''){
            setBookersRoom({name: form.values.name, family: form.values.family, mobile: form.values.mobile, entryDate: entryDate, stayingTimeHotel:stayingTime})
            createUsersRooms({name: form.values.name, family: form.values.family, mobile: form.values.mobile, nationalCode: form.values.nationalCode, email: form.values.email, stayingTimeHotel: stayingTime, entryDate: entryDate, roomsSelected: selectedMyRoom})
            navigate('/displayrezervationinfo')
        }else{
                                ...
                                selectedMyRoom.map((room) =>(
                                   ...
                                   <>
                                        ...
                                        <div className='totalPriceBox'>
                                            <span>{rooms.map(item => item.id === room.id && item.qty * room.price * night)} تومان</span>
                                        </div>
