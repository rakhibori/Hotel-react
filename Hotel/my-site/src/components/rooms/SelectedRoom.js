...
const SelectedRoom = ({setFlagRezerv, selectedMyRoom, night}) => {
    ...
    const handlePlusCount = async(room)=>{ 
        var countRoom = 1;
        var countCapacity = 0;
        const temp = roomData.filter(item => item.id === room.id)
                   ...
                   <>
                        <div className='selectedRoomBox'>
                            <div className='namePriceBox'>
                                ...
                                <div className='price'>{roomData.map(item => item.id === room.id && item.qty * room.price * night)} تومان</div>
                            </div>
                            <div className='changeCountBox'>
                                <button onClick={()=>handlePlusCount(room)}>+</button>
                                {
                                    roomData.map(item => item.id === room.id && item.qty)     
                                } اتاق
