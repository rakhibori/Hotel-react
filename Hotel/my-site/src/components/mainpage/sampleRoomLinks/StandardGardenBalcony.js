...
import StandardDoubleRoomSlider from '../../roomsAndSuites/StandardDoubleRoomSlider'
import { useGetSampleRoomsQuery } from '../../../app/services/roomsApi';
import { useNavigate } from 'react-router-dom';

const StandardGardenBalcony = () => {
    const {data: rooms, isLoading, isFetching, isError, error} = useGetSampleRoomsQuery()
    const room = rooms.filter(room => room.link === '/standardgardenbalcony')
    const navigate = useNavigate()

    const handleReservThisRoom = () => {
        navigate('/standardgardenbalconysearch')
    }
...
export default StandardGardenBalcony;